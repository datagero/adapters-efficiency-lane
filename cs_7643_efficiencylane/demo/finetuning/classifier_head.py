...
def get_model_and_data(model_variant):
    # ======================================================
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config_name', type=str, default='classifier_head', help='the name of the configuration file')
    ...
    save_model_name = model_variant.split("/")[-1]

    with initialize(config_path=config_path):
        cfg = compose(config_name=config_name)
        
    model, dataset = get_model_and_data(model_variant)
    ...
    optuna_objectives.run_study_experiments(cfg, model=model, dataset=dataset, study_name=study_name)
